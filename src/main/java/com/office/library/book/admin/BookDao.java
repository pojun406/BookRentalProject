...
public class BookDao {
	...
	public int insertBook(BookVo bookVo) {
		...
		String sql = "INSERT INTO tbl_book(b_thumbnail, b_name, b_author, b_publisher, b_publish_year, "
				+ "b_isbn, b_call_number, b_rantal_able, b_reg_date, b_mod_date) "
				+ "VALUES(?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
		...
		try {
			result = jdbcTemplate.update(sql, bookVo.getB_thumbnail(), 
					bookVo.getB_name(), bookVo.getB_author(), bookVo.getB_publisher(), 
					bookVo.getB_publish_year(), bookVo.getB_isbn(), bookVo.getB_call_number(), 
		...
		try {
			bookVos = jdbcTemplate.query(sql, new RowMapper<BookVo>() {
				...
				public BookVo mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					bookVo.setB_rental_able(rs.getInt("b_rantal_able"));
	...
	public List<RentalBookVo> selectRentalBooks() {
		...
		String sql = "SELECT * FROM tbl_rental_book rb JOIN tbl_book b ON rb.b_no = b.b_no JOIN tbl_user_member um "
				+ "ON rb.u_m_no = um.u_m_no WHERE rb.rb_end_date = '1000-01-01' ORDER BY um.u_m_id ASC, rb.rb_reg_date DESC";
		...
		try {
			rentalBookVos = jdbcTemplate.query(sql, new RowMapper<RentalBookVo>() {
				...
				public RentalBookVo mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					rentalBookVo.setB_name(rs.getString("u_m_name"));
	...
	public int updateRentalBook(int rb_no) {
		...
		String sql = "UPDATE tbl_rental_book SET rb_end_date = NOW() WHERE rb_no = ?";
	...
	public int updateBook(int b_no) {
		...
		String sql = "UPDATE tbl_book SET b_rantal_able = 1 WHERE b_no = ?";
	...
	public List<RentalBookVo> selectRentalBookHistory(int u_m_no) {
		...
		String sql = "SELECT * FROM tbl_rental_book rb JOIN tbl_book b ON rb.b_no = b.b_no JOIN tbl_user_member um "
				+ "ON rb.u_m_no = um.u_m_no WHERE rb.u_m_no = ? ORDER BY rb.rb_reg_date DESC";
