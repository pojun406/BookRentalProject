...
public class UserMemberDao {
	...
	public int insertUserAccount(UserMemberVo userMemberVo) {
		...
		try {
			result = jdbcTemplate.update(sql, userMemberVo.getU_m_id(), passwordEncoder.encode(userMemberVo.getU_m_pw()),
					userMemberVo.getU_m_name(), userMemberVo.getU_m_gender(), userMemberVo.getU_m_mail(), userMemberVo.getU_m_phone());
		} catch(Exception e) {
			e.printStackTrace();
		}
		return result;
	...
	public int updateUserAccount(UserMemberVo userMemberVo) {
		...
		String sql = "UPDATE tbl_user_member SET u_m_name = ?, u_m_gender = ?, u_m_mail = ?, u_m_phone = ?, "
				+ "u_m_mod_date = NOW() WHERE u_m_no = ?";
